...
public:
	...
	void PackSamus()
	{
		...
		if (SamusPose > MorphBall || SamusPose < Morphing)
		{
			...
			MidVertOfScreen = ((2 + ((y/2)))) ;
		}

		BottomOfSamus = MidVertOfScreen;
//
	//	OamCounter = (unsigned __int8)Next_OAM_slot;


		for (unsigned int count = Next_OAM_slot; count < nextDrawSlot; count++)
		{
			OverAllOAM thisOAM;
			...
			//thisOAM.enOAM.OAM0 = thisOAM.enOAM.OAM0 & 0xFF00 | (thisOAM.enOAM.OAM0 + BottomOfSamus) & 0xFF;
