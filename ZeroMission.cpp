...
int ZeroMission::GetPalSize(int sprID)
{
	...
	rom->fread(&off, 4, 1, (FILE*)NULL);
	rom->seek(off - 0x8000000);
	rom->fread(&thiscompheader.check_ten, 1, 1, (FILE*)NULL);       //Check byte should be 0x10 for lz
	rom->fread(&thiscompheader.l3, 1, 1, (FILE*)NULL);
	rom->fread(&thiscompheader.l2, 1, 1, (FILE*)NULL);
	rom->fread(&thiscompheader.l1, 1, 1, (FILE*)NULL);
